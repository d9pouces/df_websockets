...
def _trigger_signal(
    ...
    * write messages to websockets if no delay is required
    ...
    background_kwargs = {}
    if expires:
        background_kwargs["expires"] = expires
    if eta:
    ...
    if background_kwargs and serialized_client_topics:
...
def call_celery_task(args, queue, expires=None, eta=None, countdown=None):
    from df_websockets.tasks import _server_signal_call

    _server_signal_call.apply_async(
        *args, queue=queue, expires=expires, eta=eta, countdown=countdown
    )
